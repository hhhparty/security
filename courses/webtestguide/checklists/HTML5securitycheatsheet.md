...
postmessage 允许每个window（包括当前窗口、弹出窗口、iframe等）对象往其他窗口发送文本消息，从而实现跨窗口的消息传递。这个功能不受同源策略限制。

window这个对象本身也不怎么受同源策略限制。很多攻击都使用了window对象。
...
Web Storage类似一个关系数据库，由Key-value对组成，可以通过js操作。
- 设置一个值：`window.sessionStorage.setItem(key,value);`
- 读取一个值：`window.sessionStorage.getItem(key);`


Web Storage 分为：
- session storage：关闭浏览器后消失； 
- local storage：一直存在。
- firefox 还有 globalStorage

webstorage 受同源策略的约束，每个域拥有自己的信息。它使web开发更灵活，但也为xss payload开了方便之门。web storage不应当存放敏感信息。特别是local 型的。
...
- For HTML links:
    ...
    - To also remove the referrer information use this attribute value: `rel="noopener noreferrer"`.标签制定了noreferrer后，浏览器在请求该标签指定的地址时，将不再发送referrer。隐藏了敏感信息。
...
- 对于不受信任的内容，在 `iframe` 标签中使用 `sandbox` 属性。
