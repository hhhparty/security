...
异步callback函数是node.js的一个重要特性。然而在callback函数中过多的嵌套层可能是有问题的。任何多阶段的过程，可能会嵌套10层或更多，这个问题被称为 Pyramid of Doom 或 Callback hell。在这种代码中，错误和结果会丢失在嵌套中，Promises 提供了一种比callback好的异步实现机制，是从上到下的执行的将错误提交给下一个函数。
...
缓存和解析请求可能需要占用server上的内存空间，如果不对请求进行限定，攻击者可能会发送带有巨大请求body的请求，耗尽资源或占满磁盘。但对所有请求都固化大小似乎也不是正确的行为，例如上传文件。

此外，带有json类型的input比多个input更加危险，因为解析json是一个阻塞型操作。

所以，应该对不同的请求大小根据类型进行限制。
