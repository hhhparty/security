...
#### 摄像头组成介绍
Camera 在汽车上的应用场景有：
- 前视系统：前视系统一般需要 1~3 颗摄像头模组，有单目前视（单颗摄像头），双目前视（两颗摄像头），也有少数三目摄像头（三颗摄像头），一般安装在车内的前挡风玻璃的正中间。
- 环视系统：一般需要 4 ~6 颗摄像头模组，分部在车身四周，由于观测角度及范围的要求，需要使用至少  120° 的广角摄像头。
- 倒车影像：一般需要一颗广角摄像头，对像素要求并不是很高，安装在车尾中间位置，由于观测的角度及观测的范围不同，一般不能和环视系统车后端的摄像头共用。
- 驾驶员监测：位于车内，一般需要 1~2 颗摄像头模组，根据摄像头是识别人脸还是识别眼球，来决定摄像头模组的像素，并通过算法完成对驾驶员行为的监测。

Camera从结构上，包括：图像传感器、MIPI连接器、Serdes、存储器等构成
##### Sensor
目前摄像头传感器有三家原厂：
- OmniVision 简称OV，豪威科技，汽车市场上推出的sensor是 OV10635，OV10640，OV9716
- ONSemi 或 Aptina，ON推出的汽车sensor是 AR0132，AR0135，AR0144等
- SONY在汽车市场推出的传感器为 IMX224等。

传感器需要由Camera 模组厂来生产，对应的模组厂有：
- 舜宇维森
- 信利
- 欧菲光
- 邱太科技
- 豪恩
##### Serdes
由于 Camera 模组进来的图像需要进行远距离的传输（2米 以上），为保证图像在传输过程中不丢失，就会用到 Serdes，Serdes是 SERializer(串行器)/DESerializer(解串器)的简称，它是一种主流的时分多路复用(TDM)、点对点(P2P)的串行通信技术。Serdes 是成对使用的，最大的传输距离可以达到 15 米，目前市场上有 Serdes 的只有三家， TI 、 MAXIM， THine，市场上主流的是 TI 和 MAXIM的 芯片。
- TI 的产品比较全，有 DS90UB913、DS90UB914；DS90UB933、DS90UB934，DS90UB953、DS90UB954，DS90UB960 等等；
- MAXIM 的产品有 MAX96705、MAX96706，MAX9271，MAX9286 等等；
- THine 的产品有 THCV231-Q、THCV235-Q、THCV241-Q 等等。

当然，三个不同厂家的芯片，对就的协议不同，但都可以实现将图像进行长距离传输。
##### Memory 部分
Memory 部分的厂家比较多，在汽车市场上主流有 Micron，Toshiba，ISSI，Nanya 等品牌，这两年市场上的存储由于供货不足，存储的价格不断走高，Micron 退出小容量存储，也加剧了小容量存储的价格上升。
...
#### 物理层
ADAS与摄像头等图像、视频类传感器通信，通常采用 LVDS（Low Voltage Differential Signaling）小振幅差分信号技术，他使用非常低的幅度信号（250mV～450mV）通过一对平行的PCB走线或平衡电缆传输数据。能够提供 840Mbps～几Gbps的传输速率。

> LVDS在两条平行的差分信号线上流经的电流及电压振幅相反，噪声信号同时耦合到两条线上，而接受端只关心两信号的差值，于是噪声被抵消。由于两条信号线周围的电磁场也相互抵消，故差分信号传输比单线信号传输电磁辐射小得多。此外，该传输标准采用电流模式驱动输出，不会产生振铃和信号切换所带来的尖峰信号，具有良好的EMI特性。

#### 链路层
##### GMSL
GMSL(Gigabit MultimediaSerial Link)吉比特多媒体串行链路，是串行器和解串器构成的传输链路，基于LVDS传输，很多域控制器上直接使用GMSL4通道解串器，同时支持4路摄像头数据传入，例如在AVM(Around View Monitor)中，需要多路摄像输入。

主推公司为美信，最高带宽3.12Gbps，支持STP或同轴电缆双向传输，传输有方向，分camera永和显示用两种类型。
...
##### APIX
主推公司Inova ，最高带宽 12Gbps，双通道，支持STP、QSTP或同轴电缆双向传输，通道率6Gbps，数据类型支持音视频、网口、IIC、SPI、GPIO等配置数据。
...
<img src="自动驾驶/images/protocols/APIX-1.jpeg">

发送芯片和接收芯片是一对，下行带宽向下兼容，可配置为6Gbps+6Gbps、3Gbps+3Gbps或1.5Gbps+1.5Gbps，上行带宽可固定为187.5Mbps，交互的数据有多路的音视频、多路以太网和SPI、低延时的GPIO等等。该方案适合具有符合数据流的综合系统，主要的器件有INAP563T/R；INAP592T/R；INAP593T/R等，详细选型表参见Inova官网。

##### FPD-Link
主推公司为TI，最高带宽 4.16Gbps，支持STP、QSTP或同轴电缆双向传输，传输有方向，分camera永和显示用两种类型。
...
<img src="自动驾驶/images/protocols/fpd-link.jpeg">
...
<img src="自动驾驶/images/protocols/fpd-link3.jpeg">

##### ClocklessLink
主推公司为 罗母，最高带宽2.7Gbps，支持STP或同轴电缆双向传输，传输有方向，分camera永和显示用两种类型。

##### 接口小结

这些接口本质上都是一种串行解串器，跟我们FPGA的熟知的GTP、GTX这些并没有什么本质差别；
这些接口大多可支持同轴电缆传输，而且支持POC供电；
针对车用可靠性要求高的特点增加一些特性，比如说直流均衡、比如扩频技术、比如说加强了的CDR等等；
这些接口中使用最为广泛的是FPD-LINK III和GMSL，提供的功能最为丰富灵活的是APIX接口；
从设计角度而言，就是在原有的Camera、主机或屏接口外面套一层接口，将技术风险降到了最低；从部署上来说，一根同轴电缆即可连接一个Camera或一块屏（功耗允许时），布线复杂度降到了最低.
...
https://zhuanlan.zhihu.com/p/344395751

## ADAS 域控制器需求

- 功能需求：（1）主动安全功能：车道偏离预警LDW、车道保持系统LKS、前碰撞预防系统FCW 、行人碰撞预警PCW、自动紧急制动AEB、盲点监测系统BSD 、变道辅助LCA、后向碰撞预警RCW、车道居中控制LCC；（2）驾驶舒适性：驾驶员疲劳预警系统DFM、自适应巡航系统ACC、自适应灯光控制ALC、交通拥堵辅助TJA、集成巡航辅助ICA、高速公路辅助HWA、夜视系统NVD、导航自动驾驶辅助NOA；（3）泊车辅助：自动泊车系统APA、远程泊车辅助RPA、家庭区域泊车辅助HPA、全景环视监控AVM、召唤Summon。
- 与摄像头通信：最好支持 APIX v3协议；或支持FPD-LINK III和GMSL协议。
- 与高静地图通信L：支持ADASIS v3
- 总线通信：（1）支持XCP标定协议；（2）支持CAN及CANFD；（3）支持车载以太网